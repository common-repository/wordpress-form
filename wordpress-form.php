...
Description: Wordpress form plugin creates a form on any page, post, or sidebar of your website. Simply insert [WORDPRESS-FORM] within any post or page.
...
function ContactUsForm()
{
	...
	add_shortcode('WORDPRESS-FORM', array( &$this, 'shortcode'));

	add_action('wp_head', array( &$this, 'addStyle'));

	if ( function_exists('register_uninstall_hook') )
		register_uninstall_hook(ABSPATH.PLUGINDIR.'/wordpress-form/wordpress-form.php', array( &$this, 'uninstall')); 
...
function showForm( $params = '' )
{
	...
	if ( empty($result) || (!empty($result) && !$this->o['hideform']) )
	{
		...
		$form .= '
			...
			<textarea name="cuf_msg'.$n.'" id="cuf_msg'.$n.'" class="cuf_textarea">'.$cuf_msg.'</textarea>
			';
		if ( $this->o['captcha'] )
		...
		$form .= '	
			...
			<a href="https://www.kenmoredesign.com/forex" title="Forex Web Design" alt="Forex Web Design" target="_blank">Forex Web Design</a>
			</div>
...
{	
	...
	if ( isset($_POST['cuf_save']) )
	{
		...
		$this->o = array(
			...
			'color'		=> stripslashes($_POST['cuf_color']),
			'size'		=> stripslashes($_POST['cuf_size']),
			'hideform'			=> $hideform
			);
	...
	<div id="poststuff" class="wrap">
    	...
    	<tr>
	    <th>   </th>
	    	<td>To insert the form on the page simply pate the following shortcode in the HTML: <b>[WORDPRESS-FORM]</b></td>
		<tr>
    	...
    	<tr>
			...
			<td>
				<p><?php _e('The Wordpress form includes the fields Name, Email, Subject and Message. To add more fields simply add them below', 'cuf-lang'); ?></p>
				<?php
				for ( $x = 1; $x <= 5; $x++ )
		...
		<tr>
			<th>Color's Button</th>
			<td>
				<select name="cuf_color" style="width:200px;">
					<option <?php echo ((isset($this->o['color']) && $this->o['color'] == 'green') ? 'selected' : '');  ?> value="green">Green</option>
					<option <?php echo ((isset($this->o['color']) && $this->o['color'] == 'blue') ? 'selected' : '');  ?> value="blue">Blue</option>
					<option <?php echo ((isset($this->o['color']) && $this->o['color'] == 'grey') ? 'selected' : '');  ?> value="grey">Grey</option>
				</select>
			</td>
		</tr>
		<tr>
			<th>Form's Size</th>
			<td>
				<select name="cuf_size" style="width:200px;">
					<option <?php echo ((isset($this->o['size']) && $this->o['size'] == 'small') ? 'selected' : '');  ?> value="small">Small</option>
					<option <?php echo ((isset($this->o['size']) && $this->o['size'] == 'medium') ? 'selected' : '');  ?> value="medium">Medium</option>
					<option <?php echo ((isset($this->o['size']) && $this->o['size'] == 'large') ? 'selected' : '');  ?> value="large">Large</option>
				</select>
			</td>
		</tr>
		</table>
...
function addOptionsPage()
{
	...
	$menutitle .= 'Wordpress Form';
	add_options_page('Wordpress Form', $menutitle, 9, 'contact-us-form', array( &$this, 'optionsPage'));
}
...
function addStyle()
{
	if ($this->o['css']) {
		...
		echo "\n<!-- Contact Us Form -->\n"
			...
			.".cuf_input {display:none !important; visibility:hidden !important;}\n"
			;
if (isset($this->o['color']) && $this->o['color'] == 'blue') {
	echo "#contactsubmit:hover, #contactsubmit:focus {
...
} else if (isset($this->o['color']) && $this->o['color'] == 'grey') { 
	echo "#contactsubmit:hover, #contactsubmit:focus {
	background: #849F00 repeat-x;
	color: #FFF;
	text-decoration: none;
...
if (isset($this->o['size']) && $this->o['size'] == 'small') { echo "
#tinyform {
clear: both;
width:150px;
	margin-left:auto;
...
"; } else if (isset($this->o['size']) && $this->o['size'] == 'medium') { echo "
#tinyform {
clear: both;
width:300px;
	margin-left:auto;
...
function getCaptcha( $n = '' )
{
	...
	return '<input name="'.base64_encode(strrev('current_time')).'" type="hidden" value="'.base64_encode(strrev(time())).'" />'."\n"
		...
		.'<label class="cuf_label" style="display:inline" for="cuf_captcha'.$n.'">'.$contact_us_form->o['captcha_label'].' <b>'.$this->getQuestion().'</b></label> <input class="cuf_field_1" id="cuf_captcha'.$n.'" name="'.base64_encode(strrev('answer')).'" type="text"  /><br>'."\n";
}
